...
{
	if (a.nom ( ) != b.nom ( ))
		throw erreur ("Nom d'attribut différent : " + a.nom ( ) + " / " + b.nom ( ));
	if (a.type ( ) != b.type ( ))
		throw erreur ("Nom d'attribut différent : " + type2str (a.type ( )) + " / " + type2str (b.type ( )));
	if(a.nb_elements ( ) != b.nb_elements ( ))
		throw erreur ("Nombre d'éléments d'attribut différent : " + type2str (a.nb_elements ( )) + " / " + type2str (b.nb_elements ( )));
	ostringstream	str;
	if (Attribut::ENTIER == a.type ( ))
	{
		...
		for (size_t i = 0; i < a.nb_elements ( ); i++)
		{
			...
			if (i1 != i2)
			{
				str << i << "-ème valeur de l'attribut " << a.nom ( ) << " de type " << type2str (a.type ( )) << " différente ("
				    << i1 << " / " << i2 << ").";
...
static void compareAttributes (const Liste_attribut& l1, const Liste_attribut& l2)
{	
...
XlmLimaComparer::XlmLimaComparer (
				const string& inputFile1, IN_LIMA format_t inputFormat1, const IN_STD string& inputFile2, IN_LIMA format_t inputFormat2)
	: XlmLimaAction (inputFile1, inputFormat1), _inputFile2 (inputFile2), _inputFormat2 (inputFormat2)
{
}	// XlmLimaComparer::XlmLimaComparer
...
void XlmLimaComparer::run ( )
{
	try
	{
		...
		cout << "Comparaison des maillages " << getInputFile ( ) << " et " << getInputFile2 ( ) << " ..." << endl;
		...
		{
			str << "Unités de longueur différentes (" << mesh1.unite_longueur( ) << " / " << mesh2.unite_longueur ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.unite_longueur ( ) != mesh2.unite_longueur ( ))
		if (mesh1.unite_angle ( ) != mesh2.unite_angle ( ))
		{
			str << "Unités d'angles différentes (" << mesh1.unite_angle ( ) << " / " << mesh2.unite_angle ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.unite_angle ( ) != mesh2.unite_angle ( ))
		if (mesh1.type_coordonnee ( ) != mesh2.type_coordonnee ( ))
		{
			str << "Types de coordonnées différents ("
			    << typeCoord2str (mesh1.type_coordonnee ( )) << " / " << typeCoord2str (mesh2.type_coordonnee ( )) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.type_coordonnee ( ) != mesh2.type_coordonnee ( ))
		if (mesh1.type_geometrie ( ) != mesh2.type_geometrie ( ))
		{
			str << "Types de géométries différents ("
			    << typeGeom2str (mesh1.type_geometrie ( )) << " / " << typeGeom2str (mesh2.type_geometrie ( )) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.type_geometrie ( ) != mesh2.type_geometrie ( ))
		if (mesh1.dimension ( ) != mesh2.dimension ( ))
		{
			str << "Dimensions différentes (" << mesh1.dimension ( ) << " / " << mesh2.dimension ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.dimension ( ) != mesh2.dimension ( ))
		if (mesh1.nb_noeuds ( ) != mesh2.nb_noeuds ( ))
		{
			str << "Nombre de noeuds différent (" << mesh1.nb_noeuds ( ) << " / " << mesh2.nb_noeuds ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_noeuds ( ) != mesh2.nb_noeuds ( ))
		if (mesh1.nb_bras ( ) != mesh2.nb_bras ( ))
		{
			str << "Nombre de bras différent (" << mesh1.nb_bras ( ) << " / " << mesh2.nb_bras ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_bras ( ) != mesh2.nb_bras ( ))
		if (mesh1.nb_polygones ( ) != mesh2.nb_polygones ( ))
		{
			str << "Nombre de polygones différent (" << mesh1.nb_polygones ( ) << " / " << mesh2.nb_polygones ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_polygones ( ) != mesh2.nb_polygones ( ))
		if (mesh1.nb_polyedres ( ) != mesh2.nb_polyedres ( ))
		{
			str << "Nombre de polyèdres différent (" << mesh1.nb_polyedres ( ) << " / " << mesh2.nb_polyedres ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_polyedres ( ) != mesh2.nb_polyedres ( ))
		if (mesh1.nb_nuages ( ) != mesh2.nb_nuages ( ))
		{
			str << "Nombre de nuages différent (" << mesh1.nb_nuages ( ) << " / " << mesh2.nb_nuages ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_nuages ( ) != mesh2.nb_nuages ( ))
		if (mesh1.nb_lignes ( ) != mesh2.nb_lignes ( ))
		{
			str << "Nombre de lignes différent (" << mesh1.nb_lignes ( ) << " / " << mesh2.nb_lignes ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_lignes ( ) != mesh2.nb_lignes ( ))
		if (mesh1.nb_surfaces ( ) != mesh2.nb_surfaces ( ))
		{
			str << "Nombre de surfaces différent (" << mesh1.nb_surfaces ( ) << " / " << mesh2.nb_surfaces ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_surfaces ( ) != mesh2.nb_surfaces ( ))
		if (mesh1.nb_volumes ( ) != mesh2.nb_volumes ( ))
		{
			str << "Nombre de volumes différent (" << mesh1.nb_volumes ( ) << " / " << mesh2.nb_volumes ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.nb_volumes ( ) != mesh2.nb_volumes ( ))
		...
		for (i = 0; i < mesh1.nb_noeuds ( ); i++)
		{
			...
			if (n1.id ( ) != n2.id ( ))
			{
				str << "Les " << i << "-ièmes noeuds n'ont pas les mêmes identifiants : " << n1.id( ) << "/" << n2.id( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (n1.id ( ) != n2.id ( ))
			if (n1 != n2)
			{
		...
		for (i = 0; i < mesh1.nb_bras ( ); i++)
		{
			...
			if (b1.id ( ) != b2.id ( ))
			{
				str << "Les " << i << "-ièmes bras n'ont pas les mêmes identifiants : " << b1.id( ) << "/" << b2.id( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (b1.id ( ) != b2.id ( ))
			if (b1.nb_noeuds ( ) != b2.nb_noeuds ( ))
			{
				str << "Les bras " << b1.id ( ) << " n'ont pas le même nombre de noeuds.";
				throw XlmLimaException (str.str ( ));
			}	// if (b1.nb_noeuds ( ) != b2.nb_noeuds ( ))
			for (j = 0; j < b1.nb_noeuds ( ); j++)
			{
				...
				{
					str << "Les bras " << b1.id ( ) << " n'ont pas les mêmes noeuds.";
					throw XlmLimaException (str.str ( ));
				}	// if (b1.noeud (j).id ( ) != b2.noeud (j).id ( ))
			}	// for (j = 0; j < b1.nb_noeuds ( ); j++)
		}	// for (i = 0; i < mesh1.nb_bras ( ); i++)
		...
		for (i = 0; i < mesh1.nb_polygones ( ); i++)
		{
			...
			if (p1.id ( ) != p2.id ( ))
			{
				str << "Les " << i << "-ièmes polygones n'ont pas les mêmes identifiants : " << p1.id( ) << "/" << p2.id( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (p1.id ( ) != p2.id ( ))
			if (p1.nb_noeuds ( ) != p2.nb_noeuds ( ))
			{
				str << "Les polygones " << p1.id ( ) << " n'ont pas le même nombre de noeuds.";
				throw XlmLimaException (str.str ( ));
			}	// if (p1.nb_noeuds ( ) != p2.nb_noeuds ( ))
			for (j = 0; j < p1.nb_noeuds ( ); j++)
			{
				...
				{
					str << "Les polygones " << p1.id ( ) << " n'ont pas les mêmes noeuds.";
					throw XlmLimaException (str.str ( ));
				}	// if (p1.noeud (j).id ( ) != p2.noeud (j).id ( ))
			}	// for (j = 0; j < p1.nb_noeuds ( ); j++)
		}	// for (i = 0; i < mesh1.nb_polygones ( ); i++)
		...
		for (i = 0; i < mesh1.nb_polyedres ( ); i++)
		{
			...
			if (p1.id ( ) != p2.id ( ))
			{
				str << "Les " << i << "-ièmes polyèdres n'ont pas les mêmes identifiants : " << p1.id( ) << "/" << p2.id( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (p1.id ( ) != p2.id ( ))
			if (p1.nb_noeuds ( ) != p2.nb_noeuds ( ))
			{
				str << "Les polyèdres " << p1.id ( ) << " n'ont pas le même nombre de noeuds.";
				throw XlmLimaException (str.str ( ));
			}	// if (p1.nb_noeuds ( ) != p2.nb_noeuds ( ))
			for (j = 0; j < p1.nb_noeuds ( ); j++)
			{
				if (p1.noeud (j).id ( ) != p2.noeud (j).id ( ))
				{
					str << "Les polyèdres " << p1.id ( ) << " n'ont pas les mêmes noeuds.";
					throw XlmLimaException (str.str ( ));
				}	// if (p1.noeud (j).id ( ) != p2.noeud (j).id ( ))
			}	// for (j = 0; j < p1.nb_noeuds ( ); j++)
		}	// for (i = 0; i < mesh1.nb_polyedres ( ); i++)
...
/*			Nuage	n1	= mesh1.nuage (i), n2	= mesh2.nuage (i);
			if (n1.nom ( ) != n2.nom ( ))
			{
				str << "Les " << i << "-ièmes nuages n'ont pas les mêmes noms : " << n1.nom( ) << "/" << n2.nom( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (n1.nom ( ) != n2.nom ( ))	*/
			Nuage	n1	= mesh1.nuage (i), n2	= mesh2.nuage (n1.nom( ));	// v 7.7.9
			if (n1.nb_noeuds ( ) != n2.nb_noeuds ( ))
			{
				str << "Les nuages " << n1.nom ( ) << " n'ont pas le même nombre de noeuds (" << n1.nb_noeuds ( ) << "/" << n2.nb_noeuds ( ) << ").";
				throw XlmLimaException (str.str ( ));
			}	// if (n1.nb_noeuds ( ) != n2.nb_noeuds ( ))
			for (j = 0; j < n1.nb_noeuds ( ); j++)
			{
				...
				{
					str << "Les " << j << "-ièmes noeuds du nuage " << n1.nom ( ) << " n'ont pas les mêmes identifiants ("
...
/*			Ligne	l1	= mesh1.ligne (i), l2	= mesh2.ligne (i);
			if (l1.nom ( ) != l2.nom ( ))
			{
				str << "Les " << i << "-ièmes lignes n'ont pas les mêmes noms : " << l1.nom( ) << "/" << l2.nom( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (l1.nom ( ) != l2.nom ( ))	*/
			Ligne	l1	= mesh1.ligne (i), l2	= mesh2.ligne (l1.nom ( ));	// v 7.7.9
			if (l1.nb_bras ( ) != l2.nb_bras ( ))
			{
				str << "Les lignes " << l1.nom ( ) << " n'ont pas le même nombre de bras (" << l1.nb_bras ( ) << "/" << l2.nb_bras ( ) << ").";
				throw XlmLimaException (str.str ( ));
			}	// if (l1.nb_bras ( ) != l2.nb_bras ( ))
			for (j = 0; j < l1.nb_bras ( ); j++)
			{
				...
				{
					str << "Les " << j << "-ièmes bras de la ligne " << l1.nom ( ) << " n'ont pas les mêmes identifiants ("
...
/*			Surface	s1	= mesh1.surface (i), s2	= mesh2.surface (i);
			if (s1.nom ( ) != s2.nom ( ))
			{
				str << "Les " << i << "-ièmes surfaces n'ont pas les mêmes noms : " << s1.nom( ) << "/" << s2.nom( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (s1.nom ( ) != s2.nom ( ))	*/
			Surface	s1	= mesh1.surface (i), s2	= mesh2.surface (s1.nom ( ));
			if (s1.nb_polygones ( ) != s2.nb_polygones ( ))
			{
				str << "Les surfaces " << s1.nom ( ) << " n'ont pas le même nombre de polygones (" << s1.nb_polygones ( ) << "/" << s2.nb_polygones ( ) <<").";
				throw XlmLimaException (str.str ( ));
			}	// if (s1.nb_polygones ( ) != s2.nb_polygones ( ))
			for (j = 0; j < s1.nb_polygones ( ); j++)
			{
				...
				{
					str << "Les " << j << "-ièmes polygones de la surface " << s1.nom ( ) << " n'ont pas les mêmes identifiants ("
					    << s1.polygone (j).id ( ) << " / " << s2.polygone (j).id ( ) << ").";
					throw XlmLimaException (str.str ( ));
				}	// if (s1.polygone (j).id ( ) != s2.polygone (j).id ( ))
			}	// for (j = 0; j < s1.nb_polygones ( ); j++)
...
/*			Volume	v1	= mesh1.volume (i), v2	= mesh2.volume (i);
			if (v1.nom ( ) != v2.nom ( ))
			{
				str << "Les " << i << "-ièmes volumes n'ont pas les mêmes noms : " << v1.nom( ) << "/" << v2.nom( ) << ".";
				throw XlmLimaException (str.str ( ));
			}	// if (v1.nom ( ) != v2.nom ( ))	*/
			Volume	v1	= mesh1.volume (i), v2	= mesh2.volume (v1.nom ( ));	// v 7.7.9
			if (v1.nb_polyedres ( ) != v2.nb_polyedres ( ))
			{
				str << "Les volumes " << v1.nom ( ) << " n'ont pas le même nombre de polyèdres (" << v1.nb_polyedres ( ) << "/" << v2.nb_polyedres ( ) <<").";
				throw XlmLimaException (str.str ( ));
			}	// if (v1.nb_polyedres ( ) != v2.nb_polyedres ( ))
			for (j = 0; j < v1.nb_polyedres ( ); j++)
			{
				...
				{
					str << "Les " << j << "-ièmes polyèdres de la volume " << v1.nom ( ) << " n'ont pas les mêmes identifiants ("
					    << v1.polyedre (j).id ( ) << " / " << v2.polyedre (j).id ( ) << ").";
					throw XlmLimaException (str.str ( ));
				}	// if (v1.polyedre (j).id ( ) != v2.polyedre (j).id ( ))
			}	// for (j = 0; j < v1.nb_polyedres ( ); j++)
		...
		{
			str << "Seuls les titres sont différents (" << mesh1.titre ( ) << " / " << mesh2.titre ( ) << ").";
			throw XlmLimaException (str.str ( ));
		}	// if (mesh1.titre ( ) != mesh2.titre ( ))

		cout << "Les maillages " << getInputFile ( ) << " et " << getInputFile2 ( ) << " sont identiques." << endl;
	}
