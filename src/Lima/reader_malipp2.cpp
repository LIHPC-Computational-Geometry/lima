...
const string    HDFAttributeDescriptor2Type::m_name ("name");

////////////////////////////////////////////////////////////////////////////////
//! Nom du champ contenant le type d'un attribut dans la structure HDF.
const string    HDFAttributeDescriptor2Type::m_type ("type");

////////////////////////////////////////////////////////////////////////////////
/*! Nom du champ contenant le nombre d'éléments d' d'un attribut dans la
    structure HDF. */
const string    HDFAttributeDescriptor2Type::m_size ("size");

//////////////////////////////////////////////////////////////////////////////
/*! Ouvre en lecture le fichier nom_fichier et ouvre le groupe correspondant au
   maillage num. */
_ReaderMalipp2::_ReaderMalipp2 (_MaillageInterne* mesh, const string& nom_fichier,
...
}	// _ReaderMalipp2::readAll

//////////////////////////////////////////////////////////////////////////////
void _ReaderMalipp2::readStructure ( )
{
	try
	{
		m_mai->debut_lecture (m_fileName, m_meshNum);
...
}	// _ReaderMalipp2::readStructure


//////////////////////////////////////////////////////////////////////////////
//! Lecture d'un fichier Malipp
void _ReaderMalipp2::lire (bool loadData)
{
	bool			throwExc	= false;
	...
	catch (const IN_H5 Exception& hdfExc)
	{
		throwExc	= true;
...
void _ReaderMalipp2::readNode (bool loadData)
{
	bool	cancel	= false;
	...
	if (true == throwError)
		throw read_erreur (errorMsg.str ( ));
}	// _ReaderMalipp2::readNode

//////////////////////////////////////////////////////////////////////////////
//! Lecture des donnees sur les bras
void _ReaderMalipp2::readEdge (bool loadData)
{
	bool	cancel	= false;
	...
	if (true == throwError)
		throw read_erreur (errorMsg.str ( ));
}	// _ReaderMalipp2::readEdge


//////////////////////////////////////////////////////////////////////////////
//! Lecture des donnees sur les polygones
void _ReaderMalipp2::readCell2D (bool loadData)
{
	bool	cancel	= false;
...
void _ReaderMalipp2::readCell3D (bool loadData)
{
	bool	cancel	= false;
...
//	m_mai->composition_polyedres (composition);	// v 6.6.0
	...
	if (true == throwError)
		throw read_erreur (errorMsg.str ( ));
}	// _ReaderMalipp2::readCell3D


//////////////////////////////////////////////////////////////////////////////
//! Lecture des donnees sur les nuages du maillage
void _ReaderMalipp2::readNodeset (bool loadData)
{
	bool			throwError	= false;
	MutableString	errorMsg;
	...
	try
	{
		...
		{
			bool	cancel	= false;
			MaliPPReader2::NuageReader	reader	= m_reader.nuage (*it);
			...
			nuage->init_att ( );
			vector<HDFAttributeDescriptor2> attributs;
			reader.lire_liste_attributs (MaliPPReader2::NOEUDS, attributs);
			size_t	count	= 1;
			vector<HDFAttributeDescriptor2>::const_iterator	ita;
			     ...
			     ita++, count++)
			{
				...
				readGroupAttr (
					reader, MaliPPReader2::NOEUDS, nuage->att_noeuds ( ), *ita);
	...
	if (true == throwError)
		throw read_erreur (errorMsg.str ( ));
}	// _ReaderMalipp2::readNodeset

//////////////////////////////////////////////////////////////////////////////
//! Lecture des donnees sur les lignes du maillage
void _ReaderMalipp2::readLine (bool loadData)
{
	bool			throwError	= false;
	MutableString	errorMsg;
	...
	try
	{
		...
		{
			bool	cancel	= false;
			MaliPPReader2::LigneReader	reader	= m_reader.ligne (*it);
			     ...
			     ita++, count++)
			{
				...
				readGroupAttr (
					reader, MaliPPReader2::NOEUDS, ligne->att_noeuds ( ), *ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
			...
			attributs.clear ( );
			// Les noeuds :
			reader.lire_liste_attributs (MaliPPReader2::BRAS, attributs);
			for (ita = attributs.begin ( ); ita != attributs.end ( );
			     ita++, count++)
			{
				m_mai->donnees_lues (m_fileName, m_meshNum,
				...
				readGroupAttr (
					reader, MaliPPReader2::BRAS, ligne->att_bras ( ), *ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
	...
	if (true == throwError)
		throw read_erreur (errorMsg.str ( ));
}	// _ReaderMalipp2::readLine

//////////////////////////////////////////////////////////////////////////////
//! Lecture des donnees sur les surfaces du maillage
void _ReaderMalipp2::readSurface (bool loadData)
{
	bool			throwError	= false;
	MutableString	errorMsg;
	...
	try
	{
		...
		{
			bool	cancel	= false;
			MaliPPReader2::SurfaceReader	reader	= m_reader.surface (*it);
			     ...
			     ita++, count++)
			{
				...
				readGroupAttr (
					reader, MaliPPReader2::NOEUDS, surface->att_noeuds( ), *ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
			     ...
			     ita++, count++)
			{
				...
				readGroupAttr (
					reader, MaliPPReader2::BRAS, surface->att_bras ( ), *ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
			...
			// Les polygones :
			reader.lire_liste_attributs (MaliPPReader2::POLYGONES, attributs);
			for (ita = attributs.begin ( ); ita != attributs.end ( );
			     ita++, count++)
			{
				m_mai->donnees_lues (m_fileName, m_meshNum,
				...
				readGroupAttr (
					reader, MaliPPReader2::POLYGONES, surface->att_polygones ( ),
					*ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
	...
	if (true == throwError)
		throw read_erreur (errorMsg.str ( ));
}	// _ReaderMalipp2::readSurface

//////////////////////////////////////////////////////////////////////////////
//! Lecture des donnees sur les volumes du maillage
void _ReaderMalipp2::readVolume (bool loadData)
{
	bool			throwError	= false;
	MutableString	errorMsg;
	...
	try
	{
		...
		{
			bool	cancel	= false;
			MaliPPReader2::VolumeReader	reader	= m_reader.volume (*it);
			     ...
			     ita++, count++)
			{
				...
				readGroupAttr (
					reader, MaliPPReader2::NOEUDS, volume->att_noeuds ( ), *ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
			     ...
			     ita++, count++)
			{
				...
				readGroupAttr (
					reader, MaliPPReader2::BRAS, volume->att_bras ( ), *ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
			     ...
			     ita++, count++)
			{
				...
				readGroupAttr (
					reader, MaliPPReader2::POLYGONES, volume->att_polygones ( ),
					*ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
			...
			// Les polyedres :
			reader.lire_liste_attributs (MaliPPReader2::POLYEDRES, attributs);
			for (ita = attributs.begin ( ); ita != attributs.end ( );
			     ita++, count++)
			{
				m_mai->donnees_lues (m_fileName, m_meshNum,
				...
				readGroupAttr (
					reader, MaliPPReader2::POLYEDRES, volume->att_polyedres ( ),
					*ita);
				double	percent	= count * 100. / attributs.size ( );
				m_mai->donnees_lues (m_fileName, m_meshNum,
	...
	if (true == throwError)
		throw read_erreur (errorMsg.str ( ));
}	// _ReaderMalipp2::readVolume


//////////////////////////////////////////////////////////////////////////////
void _ReaderMalipp2::readMeshAttr ( )
{
	...
	try
	{
		...
		m_mai->donnees_lues (m_fileName, m_meshNum,
		                     ReaderCallback::ATTR_MAILLAGE, "", 0., cancel);
		vector<HDFAttributeDescriptor2>	attributs;
		m_reader.lire_liste_attributs (MaliPPReader2::MAILLAGE, attributs);
		size_t	count	= 1;
		vector<HDFAttributeDescriptor2>::const_iterator	it;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::MAILLAGE, m_mai->att_maillage ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		                    ReaderCallback::ATTR_MAILLAGE, "", percent, cancel);
			if (true == cancel) throw _Reader::LectureAnnulee ( );
		}	// for (vector<HDFAttributeDescriptor2>::const_iterator it =

		// Les attributs des noeuds du maillage :
		m_mai->donnees_lues (m_fileName, m_meshNum,
		                     ReaderCallback::ATTR_NOEUD, "", 0., cancel);
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::NOEUDS, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::NOEUDS, m_mai->att_noeuds ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		...
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::BRAS, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::BRAS, m_mai->att_bras ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		...
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::POLYGONES, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::POLYGONES, m_mai->att_polygones ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		...
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::POLYEDRES, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::POLYEDRES, m_mai->att_polyedres ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		...
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::NUAGES, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::NUAGES, m_mai->att_nuages ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		...
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::LIGNES, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::LIGNES, m_mai->att_lignes ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		...
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::SURFACES, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::SURFACES, m_mai->att_surfaces ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
		...
		attributs.clear ( );
		m_reader.lire_liste_attributs (MaliPPReader2::VOLUMES, attributs);
		count	= 1;
		for (it = attributs.begin ( ); it != attributs.end ( ); it++,count++)
		{
			readAttr (MaliPPReader2::VOLUMES, m_mai->att_volumes ( ), *it);
			double	percent	= count * 100. / attributs.size ( );
			m_mai->donnees_lues (m_fileName, m_meshNum,
...
*/
void _ReaderMalipp2::readAttr (
					MaliPPReader2::ENTITE entite, Liste_attribut& liste,
					const HDFAttributeDescriptor2& attrDesc)
{
...
/*! Lecture d'un attribut. Cet attribut est ajouté à la liste transmise
    en argument. Le nom et le type de l'attribut sont donnés en argument.
*/
void _ReaderMalipp2::readGroupAttr (
		MaliPPReader2::GroupeReader& reader, MaliPPReader2::ENTITE entite,
		Liste_attribut& liste, const HDFAttributeDescriptor2& attrDesc)
{
	size_t		chunkSize	= 0,	first	= 0, dataCount	= 0;
	size_t		k			= 0,	ielm	= 0;
	_Attribut*	attribute	= 0;
	...
	delete [] ibuf;
	delete [] dbuf;
}	// _ReaderMalipp2::readGroupAttr


//////////////////////////////////////////////////////////////////////////////
//! Ecriture de l'indice des elements dans le champ udata
void  _WriterMalipp2::setDataToIndice ( )
{
	size_type i;
...
}	// _WriterMalipp2::setDataToIndice

//////////////////////////////////////////////////////////////////////////////
//! Mise a NULL du champ udata
void  _WriterMalipp2::setDataToNull ( )
{
	size_type i;
...
}	// _WriterMalipp2::setDataToNull

//////////////////////////////////////////////////////////////////////////////
//! Ecriture d'un fichier Malipp
void _WriterMalipp2::ecrire ( )
{
	bool			throwExc	= false;
	MutableString	errorMsg;
...
}	// _WriterMalipp2::ecrire

//////////////////////////////////////////////////////////////////////////////
//! Creation (si nécessaire) du header d'un fichier HDF5
void _WriterMalipp2::createMainHeader ( )
{
	hsize_t strDims [1];
...
}	// _WriterMalipp2::writeVersionsInfos
...
void _WriterMalipp2::nbMeshInc ( )
{
	...
	nbMeshes++;
	nbMeshesAttribute.write (PredType::NATIVE_INT, &nbMeshes);
}	// _WriterMalipp2::nbMeshInc


//////////////////////////////////////////////////////////////////////////////
//! Ecriture du header d'un maillage. Création si nécessaire.
void _WriterMalipp2::writeMeshHeader ( )
{
	Group	header	=
	...
	Attribute	meshTitleAttribute	=
		HDFHelper2::openOrCreateStringAttribute (header, MESH_TITLE_FIELD_NAME,
		                                        MLI2_ATTRIBUT_SIZE);
	HDFHelper2::writeStringAttribute (meshTitleAttribute, m_mai->titre ( ));

	// Date du maillage :
	Attribute	meshDateAttribute	=
		HDFHelper2::openOrCreateStringAttribute (header, MESH_DATE_FIELD_NAME,
		                                        MLI2_ATTRIBUT_SIZE);
	HDFHelper2::writeStringAttribute (meshDateAttribute, m_mai->date ( ));
...
void _WriterMalipp2::createMesh ( )
{
	...
	if (true == incrementMeshCount)
		nbMeshInc ( );
}	// _WriterMalipp2::createMesh

//////////////////////////////////////////////////////////////////////////////
//! Ecriture des donnees sur les noeuds
void _WriterMalipp2::writeNode ( )
{
//	Group		nodeGroup 		= m_meshGroup.openGroup (NODE_GROUP_NAME);
	Group		nodeGroup;
	try
	{
		nodeGroup 		= m_meshGroup.openGroup (NODE_GROUP_NAME);
	}
	catch (...)
	{
		if (0 == m_mai->nb_noeuds ( ))
	...
	for (size_t pos = 0; pos * chunkSize < nodeNumber; pos++)
	{
		...
		switch (m_mai->dimension ( ))
		{
			case D3	:
			{
				DataSet	zCoordDataSet	= HDFHelper2::openOrCreateDataSet1D (
				...
				HDFHelper2::writeDoubleDataSet1D (zCoordDataSet, buf, dataCount,
				                                 first);
			}
			case D2	:
			{
				DataSet	yCoordDataSet	= HDFHelper2::openOrCreateDataSet1D (
						nodeGroup, DATASET_COORD_Y_NAME, nodeNumber,
						PredType::NATIVE_DOUBLE, true, NODE_CHUNK_SIZE);
				for (ielm = first, i = 0; ielm < first + dataCount; ielm++, i++)
					buf [i] = m_mai->noeud (ielm)->y ( );
				HDFHelper2::writeDoubleDataSet1D (yCoordDataSet, buf, dataCount,
				                                 first);
			}
			case D1 :
			{
				DataSet	xCoordDataSet	= HDFHelper2::openOrCreateDataSet1D (
						nodeGroup, DATASET_COORD_X_NAME, nodeNumber,
						PredType::NATIVE_DOUBLE, true, NODE_CHUNK_SIZE);
				for (ielm = first, i = 0; ielm < first + dataCount; ielm++, i++)
					buf [i] = m_mai->noeud (ielm)->x ( );
				HDFHelper2::writeDoubleDataSet1D (xCoordDataSet, buf, dataCount,
				                                 first);
			}
			break;
			default	:
		...
		DataSet	idsDataSet	= HDFHelper2::openOrCreateDataSet1D (
		...
		if (1 == contiguous)
		{
			if (0 == pos)
			{
				...
				HDFHelper2::writeUIntDataSet1D (idsDataSet, ibuf, 1, 0);
			}	// if (0 == pos)
		...
		else
		{
			...
			HDFHelper2::writeUIntDataSet1D (idsDataSet, ibuf, dataCount, first);
		}	// else if (1 == contiguous)
	}	// for (size_t pos = 0; pos * chunkSize < nodeNumber; pos++)
	delete [] buf;
	delete [] ibuf;
}	// _WriterMalipp2::writeNode

//////////////////////////////////////////////////////////////////////////////
//! Ecriture des donnees sur les bras
void _WriterMalipp2::writeEdge ( )
{
//	Group		edgeGroup 		= m_meshGroup.openGroup (EDGE_GROUP_NAME);
	Group		edgeGroup;
	try
	{
		edgeGroup 		= m_meshGroup.openGroup (EDGE_GROUP_NAME);
	}
	catch (...)
	{
		if (0 == m_mai->nb_bras ( ))
	...
	for (size_t pos = 0; pos * chunkSize < edgeNumber; pos++)
	{
		...
		// La liste des noeuds :
		DataSet	nodeIdsDataSet	= HDFHelper2::openOrCreateDataSet1D (
				edgeGroup, DATASET_IDS_LIST_NAME, 2 * edgeNumber,
				PredType::NATIVE_UINT32, true, NODE_CHUNK_SIZE);
		HDFHelper2::writeUIntDataSet1D (nodeIdsDataSet, ibuf, nodeNumber,
		                               2 * first);

		// Ecriture des identifiants :
		size_type	count	= 0 != contiguous ? 1 : edgeNumber;
		DataSet	edgeIdsDataSet	= HDFHelper2::openOrCreateDataSet1D (
		...
		if (1 == contiguous)
		{
			if (0 == pos)
			{
				...
				HDFHelper2::writeUIntDataSet1D (edgeIdsDataSet, ibuf, 1, 0);
			}	// if (0 == pos)
		}	// if (1 == contiguous)
		else
		{
			...
			HDFHelper2::writeUIntDataSet1D (edgeIdsDataSet, ibuf, dataCount,
			                               first);
		}	// else if (1 == contiguous)
	}	// for (size_t pos = 0; pos * chunkSize < edgeNumber; pos++)

	delete [] ibuf;
}	// _WriterMalipp2::writeEdge

//////////////////////////////////////////////////////////////////////////////
//! Ecriture des donnees sur les mailles 2D
void _WriterMalipp2::writeCell2D ( )
{
//	Group		cell2DGroup 	= m_meshGroup.openGroup (CELL_2D_GROUP_NAME);
	Group		cell2DGroup;
	try
	{
		cell2DGroup 	= m_meshGroup.openGroup (CELL_2D_GROUP_NAME);
	}
	catch (...)
	{
		if (0 == m_mai->nb_polygones ( ))
	...
	for (size_t pos = 0; pos * chunkSize < cellNumber; pos++)
	{
		...
		DataSet	cellTypesDataSet	= HDFHelper2::openOrCreateDataSet1D (
				cell2DGroup, DATASET_TYPES_NAME, cellNumber,
				PredType::NATIVE_INT32, true, CELL2D_CHUNK_SIZE);
		HDFHelper2::writeIntDataSet1D (cellTypesDataSet, ibuf, dataCount, first);
		...
		DataSet	nodeIdsDataSet	= HDFHelper2::openOrCreateDataSet1D (
			...
			PredType::NATIVE_UINT32, true, NODE_CHUNK_SIZE);
		HDFHelper2::writeUIntDataSet1D (nodeIdsDataSet, uibuf, nodeNumber,
		                               totalNodeNumber - nodeNumber);
		delete [] uibuf;	uibuf	= 0;

		// Ecriture des identifiants :
		size_type	count	= 0 != contiguous ? 1 : cellNumber;
		DataSet	cellIdsDataSet	= HDFHelper2::openOrCreateDataSet1D (
		...
		if (1 == contiguous)
		{
			if (0 == pos)
			{
				...
				HDFHelper2::writeUIntDataSet1D (cellIdsDataSet, uibuf, 1, 0);
			}	// // if (0 == pos)
		}	// if (1 == contiguous)
		else
		{
			...
			HDFHelper2::writeUIntDataSet1D (cellIdsDataSet, uibuf, dataCount,
			                               first);
		}	// else if (1 == contiguous)
		delete [] uibuf;	uibuf	= 0;
	}	// for (size_t pos = 0; pos * chunkSize < cellNumber; pos++)
	delete [] ibuf;		ibuf	= 0;
}	// _WriterMalipp2::writeCell2D

//////////////////////////////////////////////////////////////////////////////
//! Ecriture des donnees sur les mailles 3D
void _WriterMalipp2::writeCell3D ( )
{
//	Group		cell3DGroup 	= m_meshGroup.openGroup (CELL_3D_GROUP_NAME);
	Group		cell3DGroup;
	try
	{
		cell3DGroup 	= m_meshGroup.openGroup (CELL_3D_GROUP_NAME);
	}
	catch (...)
	{
		if (0 == m_mai->nb_polyedres ( ))
	...
	delete [] ibuf;		ibuf	= 0;
}	// _WriterMalipp2::writeCell3D

//////////////////////////////////////////////////////////////////////////////
//! Ecriture des donnees sur les nuages du maillage
void _WriterMalipp2::writeNodeset ( )
{
//	Group		nodeSetGroup 	= m_meshGroup.openGroup (NODESET_GROUP_NAME);
	Group		nodeSetGroup;
	try
	{
		nodeSetGroup 	= m_meshGroup.openGroup (NODESET_GROUP_NAME);
	}
	catch (...)
	{
		if (0 == m_mai->nb_nuages ( ))
	...
	for (size_type grp = 0; grp < nodeSetNumber; grp++)
	{
		...
		Group	nodeSetNGroup	=
					HDFHelper2::openOrCreateGroup (nodeSetGroup, nodeSetName);

		// Ecriture du nom du nuage :
		Attribute	nameAttribute	=
			HDFHelper2::openOrCreateStringAttribute (
						nodeSetNGroup, NAME_ATTR_NAME, MLI2_ATTRIBUT_SIZE);
		HDFHelper2::writeStringAttribute (nameAttribute, nuage->nom( ));

		// Ecriture du nombre de noeuds :
		const id_type	nodeNumber	= (id_type)nuage->nb_noeuds ( );
		writeComposition (nuage->composition ( ),
								NODE_COMPOSITION_DATASET_NAME, nodeSetNGroup);

		// Ecriture des identifiants des noeuds :
		if (0 == nodeNumber)
			HDFHelper2::removeDataSet (nodeSetNGroup, DATASET_IDS_LIST_NAME);
		else
		{
			DataSet		nodeIdsDataSet	=
				HDFHelper2::openOrCreateDataSet1D (
					...
					PredType::NATIVE_UINT32, true, NODE_CHUNK_SIZE);
			size_t		chunkSize	=
							computeChunkSize (nodeNumber, sizeof(id_type));
			id_type*	uibuf = 0;
			for (size_t pos = 0; pos * chunkSize < nodeNumber; pos++)
			{
				size_t	first		= pos * chunkSize;
...
//	Group		lineGroup 	= m_meshGroup.openGroup (LINE_GROUP_NAME);
	...
	for (size_type grp = 0; grp < lineNumber; grp++)
	{
		...
		Group	lineNGroup	= HDFHelper2::openOrCreateGroup(lineGroup, lineName);

		// Ecriture du nom de la ligne :
		Attribute	nameAttribute	=
			HDFHelper2::openOrCreateStringAttribute (
						lineNGroup, NAME_ATTR_NAME, MLI2_ATTRIBUT_SIZE);
		HDFHelper2::writeStringAttribute (nameAttribute, ligne->nom( ));

		// Ecriture du nombre de bras :
		const id_type	edgeNumber	= (id_type)ligne->nb_bras ( );
		writeComposition (
			ligne->composition ( ), EDGE_COMPOSITION_DATASET_NAME, lineNGroup);
		// Ecriture des identifiants des bras :
		if (0 == edgeNumber)
			HDFHelper2::removeDataSet (lineNGroup, DATASET_IDS_LIST_NAME);
		else
		{
			DataSet		edgeIdsDataSet	=
				HDFHelper2::openOrCreateDataSet1D (
					...
					PredType::NATIVE_UINT32, true, NODE_CHUNK_SIZE);
			size_t		chunkSize	=
							computeChunkSize (edgeNumber, sizeof(id_type));
			id_type*	uibuf = 0;
			for (size_t pos = 0; pos * chunkSize < edgeNumber; pos++)
			{
				size_t	first		= pos * chunkSize;
				...
				HDFHelper2::writeUIntDataSet1D (
									edgeIdsDataSet, uibuf, dataCount, first);
			}	// for (size_t pos = 0; pos * chunkSize < edgeNumber; pos++)
			delete [] uibuf;
		}	// else if (0 == edgeNumber)
...
}	// _WriterMalipp2::writeLine

//////////////////////////////////////////////////////////////////////////////
//! Ecriture des donnees sur les surfaces du maillage
void _WriterMalipp2::writeSurface ( )
{
//	Group		surfaceGroup 	= m_meshGroup.openGroup (SURFACE_GROUP_NAME);
	Group		surfaceGroup ;
	try
	{
		surfaceGroup	= m_meshGroup.openGroup (SURFACE_GROUP_NAME);
	}
	catch (...)
	{
		if (0 == m_mai->nb_surfaces ( ))
	...
	for (size_type grp = 0; grp < surfaceNumber; grp++)
	{
		...
		Group	surfaceNGroup	=
					HDFHelper2::openOrCreateGroup (surfaceGroup, surfaceName);

		// Ecriture du nom de la surface :
		Attribute	nameAttribute	=
			HDFHelper2::openOrCreateStringAttribute (
						surfaceNGroup, NAME_ATTR_NAME, MLI2_ATTRIBUT_SIZE);
		HDFHelper2::writeStringAttribute (nameAttribute, surface->nom( ));
		...
		if (0 == polygoneNumber)
			HDFHelper2::removeDataSet (surfaceNGroup, DATASET_IDS_LIST_NAME);
		else
		{
			...
			for (size_t pos = 0; pos * chunkSize < polygoneNumber; pos++)
			{
				...
				HDFHelper2::writeUIntDataSet1D (
								polygoneIdsDataSet, uibuf, dataCount, first);
			}	// for (size_t pos = 0; pos * chunkSize < polygoneNumber; pos++)
			delete [] uibuf;
		}	// else if (0 == polygoneNumber)
...
}	// _WriterMalipp2::writeSurface

//////////////////////////////////////////////////////////////////////////////
//! Ecriture des donnees sur les volumes du maillage
void _WriterMalipp2::writeVolume ( )
{
//	Group		volumeGroup 	= m_meshGroup.openGroup (VOLUME_GROUP_NAME);
	Group		volumeGroup;
	try
	{
		volumeGroup	= m_meshGroup.openGroup (VOLUME_GROUP_NAME);
	}
	catch (...)
	{
		if (0 == m_mai->nb_volumes ( ))
	...
	for (size_type grp = 0; grp < volumeNumber; grp++)
	{
		...
		Group	volumeNGroup	=
					HDFHelper2::openOrCreateGroup (volumeGroup, volumeName);

		// Ecriture du nom du volume :
		Attribute	nameAttribute	=
			HDFHelper2::openOrCreateStringAttribute (
						volumeNGroup, NAME_ATTR_NAME, MLI2_ATTRIBUT_SIZE);
		HDFHelper2::writeStringAttribute (nameAttribute, volume->nom( ));
		...
		if (0 == polyedreNumber)
			HDFHelper2::removeDataSet (volumeNGroup, DATASET_IDS_LIST_NAME);
		else
		{
			...
			for (size_t pos = 0; pos * chunkSize < polyedreNumber; pos++)
			{
				...
				HDFHelper2::writeUIntDataSet1D (
								polyedreIdsDataSet, uibuf,dataCount, first);
			}	// for (size_t pos = 0; pos * chunkSize < polyedreNumber; pos++)
			delete [] uibuf;
		}	// else if (0 == polyedreNumber)
...
*/
void _WriterMalipp2::writeAttr (Liste_attribut* listeAttr, Group& root,
                               const string& name)
{
	HDFHelper2::removeGroup (root, name);
	if (0 == listeAttr->liste ( ).size ( ))
		return;

	Group	attrGroup	= HDFHelper2::openOrCreateGroup (root, name);
	...
	HDFAttributeDescriptor2*	attrDescList	=
						new HDFAttributeDescriptor2 [listenom.size ( )];
	     ...
	     listenom.end ( ) != iv; iv++, count++)
	{
		...
		strncpy (attrDescList [count].m_name, iv->c_str ( ), MLI2_ATTRIBUT_SIZE);
		attrDescList [count].m_name [MLI2_ATTRIBUT_SIZE - 1]	= '\0';
		...
		switch (type_att)
		{
			case Attribut::ENTIER		:
			{
				...
				for (size_t pos = 0; pos * chunkSize < nb_elements; pos++)
				{
					...
					try
					{
						HDFHelper2::writeIntDataSet1D (dataSet, ibuf, dataCount,
						                              first);
					}
					catch (...)
					{
			...
			case Attribut::CHAINE		:
			{
				...
				DataSet	dataSet	= HDFHelper2::openOrCreateDataSet1D (
	...
	DataSet		attrListDataSet	= HDFHelper2::openOrCreateDataSet1D (
					attrGroup, DATASET_ATTR_LIST_NAME, listenom.size ( ),
					attrDescrType, false);
	HDFHelper2::writeDataSet1D (attrListDataSet, attrDescList, listenom.size( ));
	delete [] attrDescList;
}	// _WriterMalipp2::writeAttr

//////////////////////////////////////////////////////////////////////////////
_WriterMalipp2::_WriterMalipp2 (_MaillageInterne* mesh, const string& nom_fichier,
                              format_t format, size_type num)
	: _Writer (mesh, nom_fichier, format),
	  m_fileName (nom_fichier), m_meshNum (num), m_hdfFile ( ), m_meshGroup ( )
{
}	// _WriterMalipp2::_WriterMalipp2
...
//////////////////////////////////////////////////////////////////////////////
void _WriterMalipp2::write (format_t format)
{
	if (MALIPP2!= format)
		throw write_erreur ("Format invalide pour ce type de writer.");

	ecrire ( );
}	// _WriterMalipp2::write


//////////////////////////////////////////////////////////////////////////////
void _WriterMalipp2::writeComposition (const Composition& composition,
                                      const string& name, Group& root)
{
...
}	// _WriterMalipp2::writeComposition
